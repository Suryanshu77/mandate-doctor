...
export function RecoveryCasesPage() { const [query,setQuery]=useState(""); const rows=cases.filter(c=>`${c.payment} ${c.customer} ${c.failure}`.toLowerCase().includes(query.toLowerCase())); return <div className="space-y-6"><PageHeader eyebrow="Operations" title="Recovery Cases" description="Inspect every failed payment, diagnosis, policy decision, and outcome." action={<Button><Zap className="size-4"/>Run recovery batch</Button>}/><Principle/><Panel><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><input value={query} onChange={e=>setQuery(e.target.value)} className="h-10 w-full rounded-lg border border-border bg-secondary pl-10 pr-3 text-sm outline-none focus:border-primary/50" placeholder="Search payment, customer, or failure..."/></label><Button variant="secondary"><Filter className="size-4"/>Filters</Button></div><CasesTable rows={rows}/></Panel></div>; }
...
export function AuditReplayPage(){const [playing,setPlaying]=useState(false);return <div className="space-y-6"><PageHeader eyebrow="Immutable decision ledger" title="Audit Replay" description="Search and replay every inference, rule evaluation, action, and financial outcome." action={<Button variant="secondary"><Download className="size-4"/>Export trace</Button>}/><Panel><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><input className="h-10 w-full rounded-lg border border-border bg-secondary pl-10 text-sm outline-none focus:border-primary/50" defaultValue="RC-2048"/></label><Button onClick={()=>setPlaying(!playing)}>{playing?<Pause className="size-4"/>:<Play className="size-4"/>}{playing?"Pause replay":"Replay decision"}</Button><Button variant="secondary"><RotateCcw className="size-4"/>Reset</Button></div></Panel><div className="grid gap-6 xl:grid-cols-[1fr_300px]"><Panel><SectionTitle title="Decision timeline" sub="Trace RC-2048 · 25 Aug 2026"/><div className="mt-6 space-y-1">{audit.map((a,i)=><div key={a.event} className={`grid grid-cols-[72px_20px_minmax(0,1fr)] gap-3 rounded-lg p-3 transition-all ${playing&&i<4?"bg-accent":""}`}><span className="pt-0.5 font-mono text-[10px] text-muted-foreground">{a.time}</span><div className="relative flex justify-center"><span className={`mt-1.5 size-2 rounded-full ${a.tone==="danger"?"bg-danger":a.tone==="ai"?"bg-ai":a.tone==="policy"?"bg-primary":"bg-warning"}`}/>{i<audit.length-1&&<span className="absolute top-4 h-12 w-px bg-border"/>}</div><div><div className="flex items-center gap-2"><p className="text-sm font-semibold">{a.event}</p>{a.event==="AI Proposal"&&<Badge status="AI"/>}{a.event==="Policy Decision"&&<Badge status="Policy"/>}</div><p className="mt-1 text-xs text-muted-foreground">{a.detail}</p></div></div>)}</div></Panel><div className="space-y-4"><Panel><History className="size-5 text-primary"/><p className="mt-4 text-sm font-semibold">Deterministic replay</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Inputs, model proposal, policy version, and action payload are preserved together.</p></Panel><Panel><p className="eyebrow">Integrity</p><div className="mt-4 flex items-center gap-2 text-sm text-success"><CheckCircle2 className="size-4"/>Trace verified</div><p className="mt-2 break-all font-mono text-[9px] text-muted-foreground">sha256:8c7f4a21…e91b</p></Panel></div></div></div>}
